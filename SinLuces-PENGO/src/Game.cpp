...
Game::Game()
{
    ...
    frameIndex = 0;  // Inicializar animaci�n
    frameTime = 0.0f;
...
void Game::FinishPlay()
{
    ...
    if (scene != nullptr) {
        ...
        scene = nullptr;  // Asegurarse de que no apunte a un lugar inv�lido
    }
...
AppStatus Game::Update()
{
    ...
    const float frameSpeed = 0.75f;  // Velocidad de la animaci�n del men�
    ...
    case GameState::MAIN_MENU: {
        frameTime += GetFrameTime(); // Calcular el tiempo de animaci�n
        if (frameTime >= frameSpeed) {
            frameTime = 0.0f;
            frameIndex = (frameIndex + 1) % totalFrames;
        }
...
void Game::Render()
{
    ...
    case GameState::MAIN_MENU:
    {
        ...
        // Dibujar la parte superior del men� con la animaci�n
        DrawTexture(*img_menu_up, 0, 0, WHITE);
        DrawTextureRec(sheet, frameRec, { 0, 0 }, WHITE); // Superponer la animaci�n sobre img_menu_up
