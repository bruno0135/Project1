...
void Player::TakeDamage(int amount)
{
	...
	// Comen�a el cooldown quan reps dany
	isDamageCooldownActive = true;
	damageCooldownTimer = 0.0f;

	// Aqu� pots posar so o animacions d'haver rebut dany
}
...
AABB Player::GetHitbox() const {
	int hitboxHeight = height; // al�ada de la hitbox
...
void Player::Update()
{
	// Intentem comen�ar a destruir un bloc si es prem la tecla corresponent
	TryDestroyBlock();

	// Si estem en proc�s de destruir un bloc, actualitzem l'animaci� de destrucci�
	if (isDestroying)
	{
		...
		// Posem el tile d'animaci� segons el frame actual
		...
		// Quan acaba l'animaci�, fem desapar�ixer el bloc i tornem a l'estat IDLE
		...
		// No fer res m�s mentre es trenca el bloc
		return;
	}
	...
	// Selecci� d'animaci� segons estat i direcci�
	switch (state)
	{
	case State::IDLE:
		switch (look)
	...
	case State::DEAD:
		// Opcional: animaci� de mort
		break;
	}

	// Actualitza l'animaci� de l'entity base
	Entity::Update();

	// Comprovaci� si s'ha complert condici� de vict�ria
	if (map && !hasWon && map->CheckDiamondLines())
	{
		hasWon = true;
		state = State::IDLE;
		Stop(); // Atura moviment i animaci�
		LOG("HAS GUANYAT");
	}
}
...
Point Player::GetFrontTilePos(int dx, int dy) const
{
	...
	{
		// Si no hi ha direcci�, centre del jugador
		frontX = box.pos.x + PLAYER_PHYSICAL_WIDTH / 2;
		frontY = box.pos.y + PLAYER_PHYSICAL_HEIGHT / 2;
	}
...
void Player::Move()
{
	...
	// Obtener posici�n del tile frente al jugador
...
void Player::TryDestroyBlock()
{
	...
	// Comprova si la tecla per trencar (E) est� pressionada
	if (IsKeyPressed(KEY_E))
	{
		// Determina la direcci� que mira el jugador
		int dx = 0, dy = 0;
		switch (look)
		{
		...
		// Obt� la posici� del tile davant el jugador
		Point frontTile = GetFrontTilePos(dx, dy);

		// Obt� el tipus de tile que hi ha davant
		Tile tileType = map->GetTile(frontTile.x, frontTile.y);

		// Si �s un bloc BLUEB, comen�a la destrucci�
		if (tileType == Tile::BLUEB)
		{
			// Canvia el tile a ICEBREAK_1 per iniciar l'animaci�
			map->SetTile(frontTile.x, frontTile.y, Tile::ICEBREAK_1);

			isDestroying = true;
			blockToDestroyTile = frontTile;
			destroyFrameCounter = 0.5;  // Reinicia el comptador d'animaci�
