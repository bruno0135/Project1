...
AppStatus Player::Initialise()
{
	...
	sprite->SetAnimationDelay((int)PlayerAnim::IDLE, ANIM_DELAY);
	sprite->AddKeyFrame((int)PlayerAnim::IDLE, { 0, 0, n, n });
	...
	for (i = 4; i < 5; ++i)
		sprite->AddKeyFrame((int)PlayerAnim::WALKING_UP, { (float)i * n, 0*n, n, n });

	for (i = 0; i < 1; ++i)
		sprite->AddKeyFrame((int)PlayerAnim::WALKING_DOWN, { (float)i * n, 0*n, n, n });
	...
	sprite->AddKeyFrame((int)PlayerAnim::CLIMBING_TOP, { 5 * n, 6 * n, n, n });
		*/
	sprite->SetAnimation((int)PlayerAnim::IDLE);
...
void Player::Stop()
{
	dir = { 0,0 };
	state = State::IDLE;
	if (IsLookingRight())	SetAnimation((int)PlayerAnim::IDLE);
	else					SetAnimation((int)PlayerAnim::IDLE);
}
...
void Player::ChangeAnimRight()
{
	...
	switch (state)
	{
		case State::IDLE:	 SetAnimation((int)PlayerAnim::IDLE);    break; 
